...
public class GarbageCollectedMemoryPoolTest {
    ...
    public void testZeroSize() throws Exception {
        new GarbageCollectedMemoryPool(0, 7, true);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testNegativeSize() throws Exception {
        new GarbageCollectedMemoryPool(-1, 7, false);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testZeroMaxAllocation() throws Exception {
        new GarbageCollectedMemoryPool(100, 0, true);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testNegativeMaxAllocation() throws Exception {
        new GarbageCollectedMemoryPool(100, -1, false);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testMaxAllocationLargerThanSize() throws Exception {
        new GarbageCollectedMemoryPool(100, 101, true);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testAllocationOverMaxAllocation() throws Exception {
        GarbageCollectedMemoryPool pool = new GarbageCollectedMemoryPool(1000, 10, false);
        pool.tryAllocate(11);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testAllocationZero() throws Exception {
        GarbageCollectedMemoryPool pool = new GarbageCollectedMemoryPool(1000, 10, true);
        pool.tryAllocate(0);
    }
    ...
    public void testAllocationBound() throws Exception {
        GarbageCollectedMemoryPool pool = new GarbageCollectedMemoryPool(21, 10, false);
    ...
    public void testBuffersGarbageCollected() throws Exception {
        ...
        Assert.assertTrue(maxSingleAllocation
            < Integer.MAX_VALUE / 2); //test JVM running with too much memory for this test logic (?)
        GarbageCollectedMemoryPool pool = new GarbageCollectedMemoryPool(maxHeap / 2, (int) maxSingleAllocation, false);
        ...
        long giveUp = System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(30);
        ...
        Assert.assertTrue("failed to allocate 30 buffers in 30 seconds", success);
    }
}
