...
/**
 * A memory pool that can reclaim its buffers when they are garbage collected, making release()ing them optional.
 */
public class GarbageCollectedMemoryPool extends SimpleMemoryPool implements AutoCloseable {
    ...
    public GarbageCollectedMemoryPool(long sizeBytes, int maxSingleAllocationSize, boolean strict) {
        super(sizeBytes, maxSingleAllocationSize, strict);
        this.alive = true;
        ...
        public void run() {
            while (alive) {
                try {
                    ...
                    //if the ref was enqueued it must theh not have been released
                    BufferMetadata metadata = buffersInFlight.remove(ref);
                    ...
                    if (metadata == null) {
                        ...
                        //up keys lazily.
                        //log.error("reclaimed buffer identity {} yet no metadata on record. this is a bug", ref.hashCode);
                        continue;
                    }

                    availableMemory.addAndGet(metadata.sizeBytes);
                    log.warn("reclaimed buffer of size {} and identity {} that was not properly release()ed", metadata.sizeBytes, ref.hashCode);
